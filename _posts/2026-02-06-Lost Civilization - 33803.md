...
### 직선형 트리
안정성이 최대인 트리는 리프노드가 2개인 직선형 트리입니다. 그래야 $0, 1, 2, 3,\ \cdots, 3, 2, 1, 0$(0이 리프노드) 형식을 갖추며 가운데 값이 가장 높은 안정성을 띄게 되기 때문입니다. 

가운데 값이 가장 높은 안정성을 띄게 되는데, 그 조건이 성립하려면 2개의 노드가 필요하게 됩니다. 따라서 이를 일반화하면 $i$번째 노드가 가장 높은 안정성을 띄려면, $\left\lfloor \frac{i}{2} \right\rfloor$개의 노드가 양쪽에 위치해줘야 합니다.

### 정렬
직선형 트리 조건을 검사하기 전에 노드를 오름차순으로 정렬합니다. 그럼 $0,0,1,1,2,2,\ \cdots$ 같이 정렬이 될테고 직선형 트리 조건이 성립한다면, 왼쪽 구간($0,1,2,\ \cdots$)과 오른쪽 구간($\cdots, 2,1,0$)을 나누고 중앙 노드로 연결을 해 줍니다.

...
### **관점 전환**
그렇다면 최소 시행으로 두 개의 구멍을 먼저 **만나게** 해 봅시다. 두 구멍이 만난 뒤에는 어느 쪽 구멍을 움직이던 모든 물건을 치우는데는 1번의 시행으로 족한 **만능 상태**가 됨을 알 수 있습니다. 그러므로 두 구멍이 **한 번에** 먼저 만나고, 나머지 한 번으로 남아있는 모든 물건을 치운다면 **최소 두 번 이내의 시행**으로 모든 물건을 치우고 만날 수 있게 됩니다.

처음에 $A$ 에서 출발해 $B$ 의 위치까지 최소 시행을 구하고, $B$ 에서 출발해 $A$의 위치까지 최소 시행을 구한 뒤 둘 중 더 작은 값이 정답이 됩니다.

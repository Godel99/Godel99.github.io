...
## **풀이**
$N$개의 $A$라는 수열 $A_1, \dots , A_N$에서 $A_{P_i} ≠ A_{P_{i+1}}$를 만족하는 순열 $P$ 중 사전 순으로 가장 앞선 순열 하나를 구해야 합니다.

### **그리디**
예를 들어 수열에 $A_1$원소가 굉장히 많다고 하면, 연속해서 $A_1$을 가져갈 수 없으므로 $A_1,\ A_2,\ A_1,\ A_3,\ A_1,\cdots$와 같이 $A_1$를 번갈아가며 선택해야 합니다. 따라서, $A_1$를 최대한 선택할 수 있는 개수는 $\frac{N}{2}+1$보다 크면 안 됩니다. 만약 더 크다면 $N$개를 넘어서는 순열이 나와 만족하는 정답이 아니므로, $-1$을 출력해 줍니다.

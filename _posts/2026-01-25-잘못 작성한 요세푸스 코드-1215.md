---
title: "[Platinum IV] 잘못 작성한 요세푸스 코드 - 1215 "
date: 2026-01-25
categories: [PS, BOJ]
tags: [수학, 정수론]
---
...
## **풀이**
### 정수 문제
이 문제는 이름만 요세푸스 문제일 뿐 실은 정수의 나눗셈 연산을 어떻게 줄일 수 있을지를 묻는 **정수론** 문제입니다.
...
### 위 수식 연산을 줄일 수 없을까?
**$i>k$ 경우**: 나머지 연산의 특성을 생각해보면 $i>k$이면 ${k}\mod{i}=k$이므로 $k\times(n-k)$를 하면 연산을 줄일 수 있습니다.

**$i<k$ 경우**: 다시 두 가지 경우로 나눠서 생각해 볼 수 있습니다.
...
### 3구간
1. $q>\sqrt{k}$ 경우 몫이 여러개가 나오므로 어쩔 수 없이 ${k}\mod{i}$ 연산을 일일이 해줍니다.
2. $q<\sqrt{k}$ 경우 동일한 몫이 여러 개 나오므로 그 구간을 한 번에 계산해 줍시다. 여기서 연산을 줄일 수 있습니다.
3. **마지막 구간은 $i>k$이므로 $k\times(n-k)$를 계산해주면 끝납니다.**
...
동일한 몫에 대한 연산의 구간을 구해서 건너뛰는 게 목적이므로 이러한 변형을 하는 것입니다.

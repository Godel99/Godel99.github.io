...
### **$N$이 홀수**
$N$이 홀수라면, ```1 2 3```, ```3 4 5```, ```5 6 7```, $\dots$ , ```N-2 N-1 N```로 나눠볼 수 있고, $\lfloor \frac{N}{2} \rfloor$개의 질문으로 $3$번부터 $N-2$번까지 점을 특정할 수 있습니다. $N$ 홀수이므로 $\lfloor \frac{N}{2} \rfloor$ 통해 1개의 질문이 남게 되고, 남은 하나의 질문으로 ```1 ? N```($5\leq?\leq N-2$)을 통해 $1$, $N$ 점을 특정하고, ```1 2 3```과 ```N-2 N-1 N```을 통해 다시 $2$번과 $N-1$번도 특정해 줍니다.
